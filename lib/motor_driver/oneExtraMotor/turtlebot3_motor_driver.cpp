...
bool Turtlebot3MotorDriver::setTorque(bool onoff)
{
  ...
  else if(dxl_error != 0)
  {
   ...
   dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, DXL_THIRD_ID, ADDR_X_TORQUE_ENABLE, onoff, &dxl_error);
  if(dxl_comm_result != COMM_SUCCESS)
  {
    Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
    return false;
...
bool Turtlebot3MotorDriver::readEncoder(int32_t &left_value, int32_t &right_value)
{
  ...
  // dxl_addparam_result = groupSyncReadEncoder_->addParam(third_motor_id_); //third motor
  // if (dxl_addparam_result != true)
  //   return false;
  ...
  // dxl_getdata_result = groupSyncReadEncoder_->isAvailable(third_motor_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION); //third motor
  // if (dxl_getdata_result != true)
  //   return false;
  ...
  // third_value = groupSyncReadEncoder_->getData(third_motor_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
...
/*
*******************************************
custom function for additional motor
send velocity to custom motor
*******************************************
*/
bool Turtlebot3MotorDriver::writeSingleVelocity(const float wheel_radius, float third_value)
{
  int64_t vel_value = (int64_t) constrain(third_value * VELOCITY_CONSTANT_VALUE / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
  ...
  data_byte[0] = DXL_LOBYTE(DXL_LOWORD(vel_value));
  data_byte[1] = DXL_HIBYTE(DXL_LOWORD(vel_value));
  data_byte[2] = DXL_LOBYTE(DXL_HIWORD(vel_value));
  data_byte[3] = DXL_HIBYTE(DXL_HIWORD(vel_value));

  dxl_addparam_result = groupSyncWriteVelocity_->addParam(third_motor_id_, (uint8_t*)&data_byte);
  if (dxl_addparam_result != true)
    return false;
