...
void loop()
{
  ...
  if ((t-tTime[8]) <= CONTROL_MOTOR_TIMEOUT) //third motor
  { 
    // motor_driver.writeSinglePosition(module_position); //position mode
    dxl.torqueOff(DXL_ID);
    dxl.setOperatingMode(DXL_ID, OP_EXTENDED_POSITION);
    dxl.torqueOn(DXL_ID);

    dxl.setGoalPosition(DXL_ID, dxl.getPresentPosition(DXL_ID) + (module_position / 0.087890625));
  }
...
void modulePositionCallback(const std_msgs::Float64& module_pos_msg)
{
  module_position = module_pos_msg.data;
  tTime[8] = millis();
}
