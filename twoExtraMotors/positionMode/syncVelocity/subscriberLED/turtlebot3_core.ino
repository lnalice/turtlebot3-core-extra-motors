...
void modulePositionCallback(const std_msgs::Float64MultiArray& mdl_pos_msg) //Extended Position Mode
{
  ...
  // Update present position in dxl_cur_pos_list
  for (i = 0; i < recv_cnt; i++) {
    uint8_t index = sr_infos.p_xels[i].id - 3; //DXL_ID starts 3(index = 0)
    dxl_cur_pos_list[index] = sr_data[i].present_position;
  }

    // Update goal position 
  for (i = 0; i < DXL_ID_CNT; i++) {
    sw_data[i].goal_position = dxl_cur_pos_list[i] + (mdl_pos_msg.data[i] / DYNAMIXEL_DEG_PER_PULSE);
    sw_data[i].goal_position = dxl_cur_pos_list[i] + (mdl_pos_msg.data[i] / DYNAMIXEL_DEG_PER_PULSE);
  }

  // SyncWrite Goal Position of motors
  dxl.syncWrite(&sw_infos);
}

/*******************************************************************************
* [CUSTOM] SETUP SYNCREAD & SYNCWRITE
* - Fill the members of structure to syncRead using external user packet buffer
* - Fill the members of structure to syncWrite using external user packet buffer
*******************************************************************************/
void updateSyncStructure() {
  // Fill the members of structure to syncRead using external user packet buffer
